...
SELECT STR_TO_DATE(CMPLNT_FR_DT, '%m/%d/%Y') as FR_DATE, count(*) as Num_Crimes, KY_CD as Num_Crimes from MyData where CMPLNT_FR_DT <> '' and (STR_TO_DATE(MyData.CMPLNT_FR_DT, '%m/%d/%Y') >= '2005-01-01') and (STR_TO_DATE(MyData.CMPLNT_FR_DT, '%m/%d/%Y') < '2017-01-01') group by FR_DATE, KY_CD;

select * from from_day_count order by FR_DATE;

create table inter_C_rpt SELECT STR_TO_DATE(RPT_DT, '%m/%d/%Y') as RPT_DATE, KY_CD from MyData where RPT_DT <> '' and (STR_TO_DATE(MyData.RPT_DT, '%m/%d/%Y') >= '2005-01-01');# group by RPT_DATE, KY_CD;





create table C109_rpt_day_count 
SELECT RPT_DATE, count(*) as Num_Crimes from inter_C_rpt where KY_CD = 109 group by RPT_DATE, KY_CD; 


drop table C341_rpt_temp;
create table C109_rpt_temp
select RPT_DATE, Num_Crimes, AVG_TMP from C109_rpt_day_count, TEMPERATURE where RPT_DATE = W_Date;

select * from C578_rpt_day_count;
...
drop table C678_rpt_temp_monthly;
create table C109_rpt_temp_monthly
select month(RPT_DATE), year(RPT_DATE), sum(Num_Crimes) as Num_Crimes, sum(AVG_TMP) as AVG_TMP 
from C109_rpt_temp group by month(RPT_DATE), year(RPT_DATE);
